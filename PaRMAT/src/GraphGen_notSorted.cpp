...
std::vector<std::pair<int64_t, int64_t>> GraphGen_notSorted::GenerateGraph(
                                       const unsigned long long nEdges,
                                       const unsigned long long nVertices,
                                       const double RMAT_a, const double RMAT_b, const double RMAT_c,
                                       const unsigned int nCPUWorkerThreads,
                                       ...
                                       )
{
    ...
    std::vector<Square> squares ( 1, Square( 0, nVertices, 0, nVertices-1, nEdges, 0, 0, 0 ) );
    ...
    std::vector<std::pair<int64_t, int64_t>> allEdges;
    allEdges.resize(nEdges);
    std::atomic<int64_t> curEdges(0);
    auto eachThreadGenEdges = [&] ( unsigned int puId )
    {
        std::vector<Edge> edgeVector;
        std::random_device rd;
        std::mt19937_64 gen(rd());
        ...
        for( unsigned int recIdx = puId; recIdx < squares.size(); recIdx += nWorkerThreads ) {
            ...
            int64_t curPos = std::atomic_fetch_add(&curEdges, (int64_t)edgeVector.size());
            for(int64_t idx = 0; idx<edgeVector.size(); idx++)
            {
                allEdges[curPos++] = std::make_pair(edgeVector[idx].src, edgeVector[idx].dst);
            }
            progressBar();
            edgeVector.clear();    // clean the edge vector for the next round
        }
    ...
    // Create threads and assign their tasks
    for( unsigned int puIdx = 0; puIdx < nWorkerThreads; ++puIdx )
        threads.push_back( std::thread( eachThreadGenEdges, puIdx ) );
